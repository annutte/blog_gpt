...
def generate_content(topic: str):
    ...
    try:
        ...
        title = openai.ChatCompletion.create(
            ...
            messages=[{
                "role": "user", 
                "content": f"Придумай интригующий заголовок для статьи на тему '{topic}', с учётом актуальных новостей:\n{recent_news}. Заголовок должен быть не заезженным и ясно передавать суть темы."
            }],
            max_tokens=60,  # Ограничиваем длину ответа
            temperature=0.6,  # Умеренная случайность
        ...
        post_content = openai.ChatCompletion.create(
            ...
            messages=[{
                ...
                6. Текст должен быть легким для восприятия"""
            }],
            max_tokens=700,  # Лимит токенов для развернутого текста
            temperature=0.5,
